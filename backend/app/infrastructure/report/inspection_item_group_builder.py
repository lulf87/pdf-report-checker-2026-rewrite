    ...
    def _build_group(
        ...
        *,
        ...
        group = InspectionItemGroup(
            ...
            effective_single_conclusion=_first_effective_value(rows, "conclusion"),
            effective_remark=_first_effective_value(rows, "remark"),
...
def _normalized_item_no(item: InspectionItem) -> tuple[str | None, bool, str | None]:
    ...
    if raw:
        ...
        digits = re.search(r"\d+", _compact(raw))
        if item.is_continuation and digits:
            return str(int(digits.group(0))), True, raw
