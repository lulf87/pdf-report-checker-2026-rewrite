...
### T-CODEX-ROBUST-01：missing target retry 与 C04 scope 防回退

- C04 已补齐 supporting equipment scope：配合使用设备不输出主样品标签缺失或字段缺失 finding，只在 coverage 中记录 `supporting_equipment_skipped`。
- 这避免 2797 第 8 页“本次检验配合使用”设备被构造成 C04 required Codex targets。
- Codex parser/service 已增加 missing-target recovery：parser 保留已返回 review，service 仅重试缺失 targets；retry 成功后继续 mandatory finalization。
- retry 失败或最终仍缺 target 时，任务仍 failed，错误码保持 `CODEX_OUTPUT_MISSING_TARGET`，不把 runtime 不完整降级为 passed。
- 用户层文案明确该错误是 LLM/Codex 复核未完成，不是报告确认错误；原始 missing target ids 仅在高级详情展示。

### T-UX-PROGRESS-01：长时间 70% 停留的可观测性修复

- 本任务不修改 C01-C11 deterministic rule 语义，也不修改 Codex finalization，只把 report-check 的运行状态拆成可展示 progress details。
- 后端 `progress_details` 会展示 upload/parse/extract/rules/evidence/codex_audit/finalize/completed/error 阶段，并保留 C01-C11 checklist。
- 规则阶段显示每个 check 的 `pending/running/passed/failed/skipped/needs_review/error`，避免 C03 等 skipped 项在 UI 中像“卡住”。
- Codex 阶段显示 current check、target type、review count、batch count、retry count 和 last retry reason；missing-target retry 会显示为 `retrying`。
- 前端进度弹层优先展示结构化 `progress_details`，旧任务没有该字段时仍回退到 `progress/current_step`。
- 质量主线后续若仍感觉慢，应结合 T-PERF 的 `performance_profile` 判断慢点；不要通过弱化 mandatory audit 或 finalization 来换取表面进度。
